...
def _get_deps(ctx):
    ...
    # add all stubs deps corresponding to a dep in all_deps
    deps_labels = [d.label for d in all_deps]
    extra_deps = [
        stub_dep
        for base_dep, stub_dep in zip(ctx.attr._stubs_keys, ctx.attr._stubs_values)
        if base_dep.label in deps_labels
    ]

    return all_deps + extra_deps
...
def _to_search_path(dirs):
    return ":".join(sorted({d: None for d in dirs if d}))
    ...
    for dep in all_deps:
        ...
        for imp in _get_py_imports(dep):
            if imp.startswith("_main/"):
                # source python files
                mypy_path.append(imp.removeprefix("_main/"))
